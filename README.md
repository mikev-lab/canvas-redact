...
* **Evidence Review Import & Export Schema (`v1.0.0`):** Bi-directional evidence manifest handling with an interactive import modal, drag-and-drop ingestion onto the video viewport, direct clipboard JSON pasting, real-time schema validation with live metadata preview cards, conflict resolution (Replace All vs. Merge with Existing), keyframe trajectory persistence, and defense-in-depth sanitization.
